...
class MovieModel():
    ...
    def add_movie(self, movie):
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute(
                    "INSERT INTO movie (id, title, duration, released) VALUES (%s, %s, %s, %s)", (movie.id, movie.title, movie.duration, movie.released))
    ...
    def update_movie(self, movie):
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute(
                    "UPDATE movie SET title = %s, duration = %s, released = %s WHERE id = %s", (movie.title, movie.duration, movie.released, movie.id))
                affected_rows=cursor.rowcount
                connection.commit()
